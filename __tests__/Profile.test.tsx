...
const ProfileStore1 = createStore(() => {
...
const ProfileStore2 = createStore(() => {
  const initialState: any = {
    themeMode: true,
    videoList: [
      {
...
describe('Write test cases on Profile Screen', () => {
  it('Check the Profile light theme Background Color', () => {
    const {getByTestId} = render(
      <Provider store={ProfileStore1}>
        <Profile />
      </Provider>,
    );
    const profileLightTheme = getByTestId('profileScreen');
    expect(profileLightTheme.props.style[1].backgroundColor).toBe('#ffffff');
  });
  it('Check the Profile Dark theme Background Color', () => {
    const {getByTestId} = render(
      <Provider store={ProfileStore2}>
        <Profile />
      </Provider>,
    );
    const profileLightTheme = getByTestId('profileScreen');
    expect(profileLightTheme.props.style[1].backgroundColor).toBe('#000000cc');
  });

  it('Click the Left Icon Back to page', () => {
    const {getByTestId} = render(
      <Provider store={ProfileStore2}>
        <Profile />
      </Provider>,
    ...
    expect(leftIcon).toBeCalled;
    expect(profileProps.navigation.goBack).toHaveBeenCalled;
  });
});
