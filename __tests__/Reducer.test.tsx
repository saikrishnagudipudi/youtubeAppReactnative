...
describe('Reducer', () => {
  it('should handle THEME action', () => {
    ...
    expect(newState.themeMode).toBe(true);
  });

  it('should handle VIDEO action', () => {
    const action = {
      type: 'VIDEO',
      payload: {id: '2'},
    };
    ...
    expect(newState.activeVideo).toBeCalled;
  });
  ...
  it('should handle HISTORY action', () => {
    ...
    const action = {
      type: 'HISTORY',
      payload: {
        item: {id: '2', videoDuration: 253, seeks: 25},
      },
    };

    const newState = Reducer(initialState1, action);
    // console.log(newState.historyList);
    act(() => {
      newState.historyList.filter(
        (each: {id: string}) => each.id !== action.payload.item.id,
      );
    });
    expect(newState.historyList).toEqual([
      {
        id: '2',
        title: 'The first Blender Open Movie from 2006',
        thumbnailUrl:
  ...
  it('should handle PLAYHISTORYVIDEO action', () => {
    const action = {
    ...
    const newState = Reducer(ReducerState, action);
    // const newArray = jest.fn();
    // console.log(newState.activeVideo);
    expect(newState.activeVideo).toBeCalled;
  });
});
